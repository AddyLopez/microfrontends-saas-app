...
const mount = (element, { onNavigate, defaultHistory }) => {
  const history = defaultHistory || createMemoryHistory(); // defaultHistory is used when running Auth app in isolation in development. Otherwise, routing for subapps will rely on memory history, while routing for container app will rely on browser history.
